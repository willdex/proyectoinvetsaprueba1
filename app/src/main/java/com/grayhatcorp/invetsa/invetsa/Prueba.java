...
public class Prueba extends AppCompatActivity implements View.OnClickListener{
    ...
    TextView tv_proceso;
    TextView tv_detalle,tv_total,tv_sincronizados;

    AlertDialog alertDialog_firmar_1;

    ArrayList<String> datos = new ArrayList<String>();
    ListView lista;
    LinearLayout ll_sincronizar;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pb_sincronizar=(ProgressBar) findViewById(R.id.pb_sincronizar);
         ...
         tv_proceso=(TextView)  findViewById(R.id.tv_proceso);
         tv_detalle=(TextView)findViewById(R.id.tv_detalle);
         lista=(ListView)findViewById(R.id.lv_lista);
         tv_total=(TextView)findViewById(R.id.tv_total);
         tv_sincronizados=(TextView)findViewById(R.id.tv_sincronizados);
        ll_sincronizar=(LinearLayout)findViewById(R.id.ll_sincronizar);


        bt_sincronizar.setOnClickListener(this);

        ip=getString(R.string.servidor);
        ll_sincronizar.setVisibility(View.INVISIBLE);
    }
    @Override
...
//1800 es 3 minutos.
               subir_formulario_mantenimiento( iteraccion_servicio_mantenimiento);
    ...
    public void subir_formulario_mantenimiento(int numero) {
        tv_proceso.setText(iteraccion+"/"+total);
        progressBar.setProgress(iteraccion);

        int id_servicio=get_id_servicio_mantenimiento(numero);
        ...
        if(jsa_servicio_mantenimiento.length()>0 && jsa_inspeccion_visual.length()>0 && jsa_inspeccion_funcionamiento.length()>0
                && jsa_detalle_inspeccion_visual.length()>0 && jsa_detalle_inspeccion_funcionamiento.length()>0)
        {
            ...
            servicio_s.execute(ip + "frmSincronizar.php?opcion=cargar_datos_servicio_mantenimiento", "1", jsa_servicio_mantenimiento.toString(),
                    jsa_inspeccion_visual.toString(), jsa_detalle_inspeccion_visual.toString(), jsa_inspeccion_funcionamiento.toString(),
                    jsa_detalle_inspeccion_funcionamiento.toString(),String.valueOf(id_servicio),nombre);
        }
        else{
            iteraccion_servicio_mantenimiento++;
            iteraccion++;
            if(iteraccion<=total && iteraccion_servicio_mantenimiento<total_servicio_mantenimiento && total_servicio_mantenimiento>0)
            {
                subir_formulario_mantenimiento(iteraccion_servicio_mantenimiento);
            }else if(iteraccion<=total && iteraccion_sistema_integral<total_sistema_integral && total_sistema_integral>0)
            {
                subir_formulario_sistema_integral(iteraccion_sistema_integral);
            }
            else if(iteraccion<=total && iteraccion_hoja_verificacion<total_hoja_verificacion && total_hoja_verificacion>0)
            {
                subir_formulario_hoja(iteraccion_hoja_verificacion);
            }else{
                finalizo();
            }
        }
    ...
    public void subir_formulario_sistema_integral(int numero){
        int id_sistema=get_id_sistema_integral(numero);
        tv_proceso.setText(iteraccion+"/"+total);
        progressBar.setProgress(iteraccion);
        ...
        if(jsa_sistema_integral.length()>0 && jsa_puntuacion.length()>0 && jsa_detalle_puntuacion.length()>0 && jsa_peso.length()>0) {
            ...
            Servicio_sincronizar servicio_s = new Servicio_sincronizar();
            servicio_s.execute(ip + "frmSincronizar.php?opcion=cargar_datos_sistema_integral", "2", jsa_sistema_integral.toString(),
                    jsa_puntuacion.toString(), jsa_detalle_puntuacion.toString(), jsa_peso.toString(),String.valueOf(id_sistema));

        }else{
            iteraccion_sistema_integral++;
            iteraccion++;
             if(iteraccion<=total && iteraccion_sistema_integral<total_sistema_integral && total_sistema_integral>0)
            {
                subir_formulario_sistema_integral(iteraccion_sistema_integral);
            }
            else if(iteraccion<=total && iteraccion_hoja_verificacion<total_hoja_verificacion && total_hoja_verificacion>0)
    ...
    public void subir_formulario_hoja(int numero){
        tv_proceso.setText(iteraccion+"/"+total);
        progressBar.setProgress(iteraccion);
        int id_hoja=get_id_hoja_verificacion(numero);
        ...
        if(js_hoja_verificacion.length()>0 && js_linea_genetica.length()>0 && js_accion.length()>0 && js_detalle_accion.length()>0
                && js_manipulacion_dilucion.length()>0 && js_mantenimiento_limpieza.length()>0&& js_linea_genetica.length()>0) {
            tv_detalle.setText("Sincronizando el formulario Hoja de Verificacón con número "+id_hoja);
            Servicio_sincronizar servicio_s = new Servicio_sincronizar();
            servicio_s.execute(ip + "frmSincronizar.php?opcion=cargar_datos_hoja_verificacion", "3", js_hoja_verificacion.toString(),
                    js_accion.toString(), js_detalle_accion.toString(), js_manipulacion_dilucion.toString(),js_mantenimiento_limpieza.toString(),
                    js_control_indice.toString(), js_linea_genetica.toString(), js_viabilidad_celular.toString(),String.valueOf(id_hoja));

        } else{
            iteraccion_hoja_verificacion++;
            iteraccion++;
            if(iteraccion<=total && iteraccion_hoja_verificacion<total_hoja_verificacion && total_hoja_verificacion>0)
            {
                subir_formulario_hoja(iteraccion_hoja_verificacion);
            }else{
                finalizo();
    ...
    public void finalizo(){
        ...
        tv_total.setText("Total de formularios:"+total);
        tv_sincronizados.setText("formularios sincronizados:"+iteraccion_correcto);
        tv_detalle.setText("");
    ...
    public class Servicio_sincronizar extends AsyncTask<String,Integer,String> {
        String s_id_hoja="",s_id_sistema_integral="",s_id_servicio_mantenimiento="",s_nombre="";
        ...
        protected void onPostExecute(String s) {
            ...
            if (s.equals("1")) {
                ...
                iteraccion_correcto++;
                iteraccion_servicio_mantenimiento++;
                datos.add("Formulario de Servicio de Mantenimiento "+s_nombre+": "+s_id_servicio_mantenimiento);
            ...
            else if(s.equals("2"))
            {
                ...
                iteraccion_correcto++;
                iteraccion_sistema_integral++;
                datos.add("Formulario de Sistema Integral : "+s_id_sistema_integral);
            ...
            else if(s.equals("4"))
            {
                ...
                iteraccion_correcto++;
                iteraccion_hoja_verificacion++;
                datos.add("Formulario de Hoja de Verificación : "+s_id_hoja);
            ...
            else
            {
               ...
               if(iteraccion>=total){
                   ...
                   mensaje_ok_cerrar("Error en tu conexion a internet. vuelve a intentarlo mas tarde.");
               }
            }
